...
module.exports = {
    ...
    async gameBan(key, rule_violation, duration, ban_alts, admin) {
        let true_duration;
        if (duration === 'perm') {
            true_duration = undefined;
        } else {
            true_duration = `${stringToDuration(duration)}s`;
        }
    ...
    async setCash(key, amount) {
        ...
        let entry = await getDatastoreEntry(
            4570608156,
            process.env.DATASTORE_NAME,
            key,
        );
        ...
        await cache.updateOne(
            { key },
            { $set: { value: entry, expiry: Date.now() + 60000 } },
        );
