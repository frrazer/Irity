      ...
      if (subcommand === "search") {
        ...
        if (!item)
          return embeds.errorEmbed(
            interaction,
            "I couldn't find that item in the Marketplace."
          );

        const bot_owned = item.serials.filter(
          (serial) => serial.u === 1
        ).length;
        const player_owned = item.serials.filter(
          (serial) => serial.u !== 1
        ).length; // this is how many the bot doesn't own
        const percent_bot_owned = (
          (bot_owned / item.serials.length) *
          100
        ).toFixed(2);
        ...
        items.forEach((item) => {
          bot_total_value +=
            (item.value || item.rap) *
            item.serials.filter((serial) => serial.u === 1).length;
        ...
        if (items.length === 0) {
          return embeds.errorEmbed(
            interaction,
            "The bot doesn't own any items right now."
          );
        }
        ...
        if (method === "value") {
          items.sort((a, b) => {
            return (
              (b.value || b.rap) *
                b.serials.filter((serial) => serial.u === 1).length -
              (a.value || a.rap) *
                a.serials.filter((serial) => serial.u === 1).length
            );
          });
        ...
        items.slice(0, 15).forEach((item, index) => {
          if (method === "value") {
            ...
            descripion += `**${index + 1}.** **${item.name}** - x${
              item.serials.filter((serial) => serial.u === 1).length
            }\n`;
          } else if (method === "best") {
            descripion += `**${index + 1}.** **${
