...
const settings = {
    monitor: new SlashCommandSubcommandBuilder()
        .setName('monitor')
        .setDescription(
            'Begin/Stop monitoring your messages for drop requests.',
        ),
    view: new SlashCommandSubcommandBuilder()
        .setName('view')
        .setDescription('View the current items in the autodropper.')
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (subcommand === 'monitor') {
            const isTracking = await autodropper.toggleTracking(
                user_id,
            ...
            return embeds.successEmbed(
                ...
                }** in this channel for drop requests.`,
            );
            ...
            const total_documents = await auto_dropper.countDocuments({
                dropped: { $ne: true },
            });
            ...
            if (page < 1 || page > total_pages) {
                return embeds.errorEmbed(
                    interaction,
                    `Please enter a number between 1 and ${total_pages}.`,
                );
            }

            const find_res = await auto_dropper
            ...
            for (const item of find_res) {
                i++;
                let item_doc = await ff_items.findOne({ ItemId: item.item_id });
                if (!item_doc) {
                    item_doc = { Name: `Unknown#${item.item_id}` };
                }

                if (item.limited_type == 'unique') {
                    string += `**${i}.** \`${
                        item_doc.Name
                    }\` - ${item.quantity.toLocaleString()}x - $${item.price.toLocaleString()}\n`;
                } else {
                    string += `**${i}.** \`${item_doc.Name}\` - ${
                        item.date
            ...
            if (!find_res[0]) {
                return embeds.errorEmbed(
                    interaction,
                    "I couldn't find an item at that index.",
                );
            }
            ...
            return embeds.successEmbed(
                interaction,
                'The item has been removed from the autodropper.',
            );
        }
    },
};
