...
module.exports = {
    async execute(interaction, client) {
        ...
        if (!options) {
            ...
            username = interaction.message.embeds[0].title
                .split(' ')[0]
                .slice(0, -2);
        } else {
            ...
            if (!user_id)
                return embeds.errorEmbed(interaction, 'User not found.');
        ...
        robloxService
            ...
            .then(async (data) => {
                ...
                try {
                    ban_status = await robloxService.getBanStatus(
                        `MAIN_${user_id}`,
                    );
                } catch (error) {
                    ban_status = 'error';
            ...
            .catch(async (error) => {
                ...
                if (
                    error.message ===
                    '404 NOT_FOUND Entry not found in the datastore.'
                ) {
                    const embed = new EmbedBuilder()
                        .setTitle(`Lookup for ${username}`)
                        .setDescription(
                            'This user has never played Arcade Haven before.',
                        )
                        .setThumbnail(thumbnail)
                        .setColor('Blue')
                        .setFooter({
                    ...
                    .addComponents(
                        new ActionRowBuilder().addComponents(
                            new TextInputBuilder()
                                ...
                                .setPlaceholder(
                                    'Which rule number did they break?',
                                )
                                .setCustomId('reason')
                                .setRequired(true)
                ...
                const transferModal = new ModalBuilder()
                    ...
                    .addComponents(
                        new ActionRowBuilder().addComponents(
                            new TextInputBuilder()
                                .setLabel(
                                    'Why are you transferring their inventory?',
                                )
                                .setPlaceholder('Reason')
                                .setCustomId('reason')
                                .setRequired(true)
        ...
        switch (customId) {
            ...
            case 'administration/lookup/transfer':
                ...
                const transfer_target =
                    interaction.fields.getTextInputValue('target');
                ...
                docs.forEach(function (item) {
                    ...
                    serials.forEach(function (serial_info, serial) {
                        ...
                        if (owner_id === Number(document.UserId)) {
                            ...
                            new_inventory[String(item.itemId)].push(
                                String(serial + 1),
                            );
                        }
                    });
                ...
                if (transfer_percent !== 100) {
                    while (total_value < target_value) {
                        const randomItem =
                            Object.keys(new_inventory)[
                                Math.floor(
                                    Math.random() *
                                        Object.keys(new_inventory).length,
                                )
                            ];
                        const randomSerial =
                            new_inventory[randomItem][
                                Math.floor(
                                    Math.random() *
                                        new_inventory[randomItem].length,
                                )
                            ];
                        ...
                        const potentialValue =
                            total_value + (item.value || item.rap || 0);
                    ...
                    items_to_transfer = Object.keys(new_inventory).map(
                        (itemId) => {
                            return new_inventory[itemId].map(
                                (serial) => `${itemId}-${serial}`,
                            );
                        },
                    );
                ...
                let update_promises = items_to_transfer.map((item) => {
                    ...
                    return items_collection.updateOne(
                        ...
                        {
                            $set: {
                                [`serials.${parseInt(serial) - 1}.u`]:
                                    target_id,
                                [`serials.${parseInt(serial) - 1}.t`]:
